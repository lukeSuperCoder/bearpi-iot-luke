...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : C2 WiFi Emoji - receive emoji number over WiFi TCP and
  *                   display on LCD. Combines C1 (ESP8266 driver) and
  *                   A14 (emoji rendering) into one auto-connect example.
  ******************************************************************************
  */
...
static void LCD_DrawHeader(void)
{
    ...
    if (server_started) {
        ...
        LCD_ShowString(5, 2, 200, 16, 16, "WiFi Emoji");
    }
    BACK_COLOR = BLACK;
}
...
int main(void)
{
  ...
  if (server_ok) {
      ...
      POINT_COLOR = GREEN;
      BACK_COLOR = BLACK;
      LCD_ShowString(20, EMOJI_Y, 200, 16, 16, "Waiting emoji...");
      POINT_COLOR = WHITE;
      LCD_ShowString(30, STATUS_Y, 180, 16, 16, "Send 1-N via TCP");
